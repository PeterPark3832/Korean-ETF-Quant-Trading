"""
멀티 전략 포트폴리오 + 시장 국면 감지 + Phase 4 팩터 강화
────────────────────────────────────────────────────────────────
...
Phase 4 추가:
  - 매크로 팩터 조정 (금리·환율 방향 기반 비중 동적 조절)
  - 변동성 타겟팅 (포트폴리오 실현 변동성 → 목표치 스케일 조정)
"""
...
from strategy.factor_engine import MacroFactorAdjuster, VolatilityTargeter
...
class MultiStrategyPortfolio(BaseStrategy):
    ...
    def __init__(
        ...
        ma_window:         int  = 200,
        use_macro_factor:  bool = True,   # Phase 4: 매크로 팩터 조정
        use_vol_targeting: bool = True,   # Phase 4: 변동성 타겟팅
    ):
        ...
        # Phase 4 모듈
        self.macro_adjuster = MacroFactorAdjuster() if use_macro_factor  else None
        self.vol_targeter   = VolatilityTargeter()  if use_vol_targeting else None
    ...
    def get_weights(self, prices: pd.DataFrame) -> pd.Series:
        ...
        # ── Phase 4: 매크로 팩터 조정 ────────────────────
        if self.macro_adjuster is not None:
            combined = self.macro_adjuster.adjust(combined, prices)

        # ── Phase 4: 변동성 타겟팅 ───────────────────────
        if self.vol_targeter is not None:
            combined = self.vol_targeter.target(combined, prices)
