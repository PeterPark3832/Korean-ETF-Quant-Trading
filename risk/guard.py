...
class RiskGuard:
    ...
    def check(self, balance: "AccountBalance") -> RiskCheckResult:
        ...
        # 3. 출금 의심 (순간적인 대규모 자산 감소 감지) 방어 로직
        # 직전 기록과 비교하여 시장 낙폭이 아닌 인위적 출금으로 보이는 급감(-15% 이상) 방어
        last_value = self._state.value_history[-1]["value"] if self._state.value_history else total
        if last_value > 0:
            tick_drop = (total - last_value) / last_value
